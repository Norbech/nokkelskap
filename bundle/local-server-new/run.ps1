    ...
    if (-not $dotnetReady) {
        ...
        if (Test-Path $bootstrapScript) {
            ...
            if (-not $dotnetExe2 -or -not (Test-RequiredDotNetRuntimes -DotNetExe $dotnetExe2)) {
                Write-Host "";
                Write-Host "Bootstrap fullfort, men .NET er ikke tilgjengelig ennÃ¥." -ForegroundColor Yellow
                Write-Host "Lukk terminalen, Ã¥pne PowerShell pÃ¥ nytt, og prÃ¸v igjen." -ForegroundColor Yellow
                exit 1
            }
        } else {
