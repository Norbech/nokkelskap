...
$launcherProject = Join-Path $repoRoot "src\\KeyCabinetApp.LocalServerLauncher\\KeyCabinetApp.LocalServerLauncher.csproj"
...
Write-Host "Publishing Launcher (UI)..." -ForegroundColor Yellow
& $dotnetExe @launcherPublishArgs
if ($LASTEXITCODE -ne 0) { throw "dotnet publish (Launcher) failed." }
...
$trayExe = Join-Path $root 'KeyCabinetServer.exe'
...
if (Test-Path $trayExe) {
    $trayAction = New-ScheduledTaskAction -Execute $trayExe -Argument "--minimized --tray --no-browser" -WorkingDirectory $root
}
...
if ($trayAction) {
    Register-ScheduledTask -TaskName $trayTaskName -Action $trayAction -Trigger $trigger -Principal $principal -Settings $settings | Out-Null
}
...
if ($trayAction) {
    Write-Host "- $trayTaskName" -ForegroundColor Green
}

if ($RunNow) {
    schtasks /Run /TN "$webTaskName" | Out-Null
    schtasks /Run /TN "$agentTaskName" | Out-Null
    if ($trayAction) { schtasks /Run /TN "$trayTaskName" | Out-Null }
...
REM Foretrukket: UI-launcher (starter serveren automatisk, uten ekstra vinduer)
...
REM Starter ogsaa tray-ikon (KeyCabinetServer.exe) hvis tilgjengelig.
...
Set-Content -Path (Join-Path $OutputDir "INSTALL-AUTOSTART.cmd") -Value $installAutostartCmd -Encoding ascii
Set-Content -Path (Join-Path $OutputDir "FJERN-AUTOSTART.cmd") -Value $fjernAutostartCmd -Encoding ascii
